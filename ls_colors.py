...
class ls_colors(ColorScheme):
    def __init__(self):
        ...
        self.ls_colors_keys_temp = [k.split('=') for k in self.ls_colors if k != '']
        self.ls_colors_keys = []
        self.tup_ls_colors = []

        # Remove duplicates, keeping last key
        # This is to make behaviour consistent with commands like 'ls' and 'tree'
        seen = set()
        for key in self.ls_colors_keys_temp[::-1]:
            if key[0] not in seen:
                self.ls_colors_keys.append(key)
                seen.add(key[0])
        self.ls_colors_keys.reverse() # Preserve previous ordering
        ...
        for key in [k for k in self.ls_colors_keys if '*.' not in k[0]]:
            if key[0] == 'fi':
            ...
            if getattr(context, key):
                ...
                for colour_val in t_attributes:
                    ...
                    # Basic colours for background
                        ...
                        colour16_bg = colour_val - 40
                    # eight more basic colours
                    elif (colour_val >= 100 and colour_val <= 107):
                        colour16_bg = colour_val - 92
